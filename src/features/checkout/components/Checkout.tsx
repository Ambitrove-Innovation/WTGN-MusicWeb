...
export function Checkout() {
  ...
  // Stable close handler so it can be referenced inside event listeners
  ...
  // Escape key + nav-link (hashchange) — close first, then scroll to target
  useEffect(() => {
    if (!isOpen) return;
    ...
    const handleHashChange = () => {
      ...
      closeCheckout();
      // Re-enable scroll before attempting to scroll to the section
      document.body.style.overflow = "";
      if (hash) {
        setTimeout(() => {
          const el = document.querySelector(hash);
          if (el) el.scrollIntoView({ behavior: "smooth" });
        }, 350); // wait for the modal exit animation (~300 ms)
      }
    };
      ...
      {isOpen && item && (
        <motion.div
          ...
          <motion.div
            ...
            {/* Close button — enlarged hit area + ESC hint */}
            <button
              onClick={closeCheckout}
              aria-label="Close checkout (Esc)"
              className="absolute top-4 right-4 z-50 flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-white/10 hover:bg-white/20 transition-colors text-white text-sm font-sans">
              <span className="text-base leading-none">✕</span>
              <span className="hidden sm:inline text-[0.65rem] tracking-[0.1em] uppercase opacity-60">
                Esc
              </span>
            </button>
            ...
            <div className="flex flex-col md:flex-row relative z-10 w-full min-h-[400px]">
              ...
              <div className="w-full md:w-1/2 p-8 md:p-12 flex flex-col justify-between">
                <div>
                  ...
                  <h2 className="font-space text-3xl font-bold mb-6">
                    Order Details
                  </h2>
                    ...
                    <div className="flex gap-4 items-center">
                      {item.img && (
                        <img
                          src={item.img}
                          alt={item.title}
                          className="w-16 h-16 object-cover bg-white/5 md:bg-black/5"
                        />
                      )}
                      <div>
                ...
                <div className="border-t border-white/20 md:border-black/10 pt-6">
                  <div className="flex justify-between items-end">
                    <span className="font-sans text-[0.75rem] uppercase tracking-[0.1em] text-white/60 md:text-black/60">
                      Total Amount
                    </span>
                    ...
                    <div className="space-y-3">
                      <button
                        ...
                        <div className="flex items-center gap-3">
                          <img
                            src="/icons/Payfast-logo.webp"
                            alt="PayFast"
                            className="w-8 h-8 object-contain"
                          />
                          <span className="font-space text-[0.8rem] font-bold tracking-[0.1em] uppercase">
                            PayFast
                          </span>
                        </div>
                        <span className="text-white/40 group-hover:text-black/40 group-hover/btn:translate-x-1 transition-transform">
                          →
                        </span>
                      </button>
                      <button
                  ...
                  <motion.div
                    ...
                    <div className="flex items-center gap-3 mb-6">
                      <button
                        ...
                        className="text-white/50 hover:text-white pb-1 border-b border-transparent hover:border-white text-sm transition-all">
                        ← Back
                      </button>
                    </div>
                    <form onSubmit={handlePay} className="space-y-4">
                      <div>
                        ...
                        <input
                          required
                          type="email"
                          className="w-full bg-white/5 border border-white/20 px-4 py-3 text-white text-sm focus:outline-none focus:border-white transition-colors placeholder:text-white/20"
                      ...
                      <div className="pt-2">
                        <button
                          type="submit"
                          className="w-full bg-white text-black py-4 font-space text-[0.8rem] font-bold tracking-[0.1em] uppercase hover:bg-white/90 transition-colors flex items-center justify-center gap-2">
                          Pay R{item.price} via{" "}
                          {paymentMethod === "payfast" ? "PayFast" : "Stripe"}
                        </button>
                      </div>
                ...
                {step === "success" && (
                  <motion.div
                    ...
                    <p className="text-white/60 text-sm mb-8 px-4">
                      {item.type === "ticket"
                        ? "Your ticket has been sent to your email with purchase details."
                        : "Payment confirmed! An email with your receipt and track details has been sent."}
                    </p>
                    <button
                      onClick={() => {
                        closeCheckout();
                        window.dispatchEvent(
                          new CustomEvent("purchase-complete", {
                            detail: item.id,
                          }),
                        );
                        if (item.type !== "ticket") {
